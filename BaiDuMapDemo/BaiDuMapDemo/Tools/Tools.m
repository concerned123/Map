...
#pragma mark - 计算文字大小
+ (CGSize)calculateTextSize:(NSString *)text maxSize:(CGSize)maxSize font:(UIFont *)fontSize {
    if (text && text.length > 0) {
        return [text boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: fontSize} context:nil].size;
    } else {
        return CGSizeZero;
    }
}
